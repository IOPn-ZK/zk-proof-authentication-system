 ...
 */
export async function getOrCreateGroup(groupId = 1, treeDepth = 20, tenantId = null) {
  try {
    // Check if database is available
    if (!db) {
      console.warn('Database not available, cannot get or create group');
 ...
 */
export async function addMemberToGroup(groupId, commitment, userEmail = null, sessionId = null, userSub = null, tenantId = null) {
  try {
    // Check if database is available
    if (!db) {
      console.warn('Database not available, cannot add member to group');
      throw new Error('Database not available');
    }
    ...
    // Update group root
    const groupData = await getOrCreateGroup(groupId, 20, tenantId);
 ...
 */
async function logAudit(action, entityType, entityId, userEmail, sessionId, metadata, success, userSub = null, tenantId = null) {
  try {
    let userEmailEnc = null;
    if (userEmail) {
      try { userEmailEnc = JSON.stringify(encrypt(userEmail)); } catch {}
    }
