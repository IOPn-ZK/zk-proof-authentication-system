...
export async function storeServerShare(auth0Sub, encryptedShare, shareHash, userId = null, tenantId = null) {
  try {
    ...
    throw new Error(`Failed to store server share: ${error.message}`);
  }
}
...
export async function storeCloudBackupMetadata(auth0Sub, cloudBackupUrl, shareHash, isEncrypted = true, userId = null, tenantId = null) {
  try {
    // Check if cloud backup metadata already exists
    const existing = await db.select()
      .from(keyShares)
      .where(and(
    ...
    if (existing.length > 0) {
      ...
      const [inserted] = await db.insert(keyShares)
        .values({
          ...
          cloudBackupEncrypted: isEncrypted,
          shareHash: shareHash,
          encryptedShare: '', // Not stored on server
          tenantId: tenantId
        })
        .returning();
    ...
    throw new Error(`Failed to store cloud backup metadata: ${error.message}`);
  }
}
...
export async function getCloudBackupMetadata(auth0Sub) {
  try {
    ...
    throw new Error(`Failed to retrieve cloud backup metadata: ${error.message}`);
  }
}
