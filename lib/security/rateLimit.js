...
export function rateLimit(type = 'default', options = {}) {
  return (req, res, next) => {
    ...
    if (!clientData || now > clientData.resetTime) {
      ...
      // Persist new window to DB asynchronously
      (async () => {
        try {
          ...
          const existing = await db.select().from(rateLimits).where(and(eq(rateLimits.identifier, identifier), eq(rateLimits.action, type))).limit(1);
          if (existing.length > 0) {
            await db.update(rateLimits)
              .set({ count: 0, windowStart, expiresAt, updatedAt: new Date() })
              .where(and(eq(rateLimits.identifier, identifier), eq(rateLimits.action, type)));
          } else {
            await db.insert(rateLimits).values({ identifier, action: type, count: 0, windowStart, expiresAt, createdAt: new Date(), updatedAt: new Date() });
          }
        } catch (e) {
          console.warn('Rate limit DB window init failed:', e?.message);
        }
      })();
    }
      ...
      (async () => {
        try {
          ...
          await db.execute`
            ...
            WHERE identifier = ${identifier} AND action = ${type}
          `;
        } catch (e) {
